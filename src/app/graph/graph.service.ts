...
export class GraphService {
  ...
  public truncateValues(arr: any, digit: number) {
    arr.forEach((obj) => {
      for (const key in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, key)) {
          const element = obj[key];
          if (!isNaN(obj[key])) {
            obj[key] = Number.parseFloat(element).toFixed(digit);
          }
        }
      }
    });
  ...
  public formatArray(params): string {
    let stringFormatter = ``;
    if (params.length > 0) {
      stringFormatter += `${params[0].name}</br>`;
    }
    params.forEach((element) => {
      stringFormatter += `${element.marker} ${element.seriesName} ${Number.parseFloat(
  ...
  public formatSingle(params): string {
    let stringFormatter = ``;
    if (params.length > 0) {
      stringFormatter += `${params[0].name}</br>`;
    }
    params.forEach((element) => {
      stringFormatter += `${element.marker} ${element.seriesName} ${Number.parseFloat(element.value).toFixed(0)}`;
      stringFormatter += '</br>';
    });
    return stringFormatter;
  }

  public getSankeyDemoData(): any {
    return {
      nodes: [
        { name: 'Focus A' },
        { name: 'Focus B' },
      ...
      links: [
        { source: 'Focus A', target: 'Environment', value: 0.342284047256003 },
        ...
        { source: 'Climate change', target: 'Vegetables (Organic)', value: 3.82757532567656e-5 },
        ...
        { source: 'Water use', target: 'Hazelnuts (Organic)', value: 1.33452642581887e-5 },
        { source: 'Water use', target: 'Cane sugar (Organic)', value: 8.78074837009238e-6 },
        { source: 'Water use', target: 'Vegetables (Organic)', value: 2.5136268682477e-6 },
        ...
        { source: 'Resource depletion', target: 'Vegetables (Organic)', value: 1.42824499436793e-5 },
        ...
        { source: 'Focus B', target: 'Human rights', value: 0.307574096993239 },
